...
3. **Automated CI/CD Pipeline**: GitHub Actions workflow for:
   - Automatic infrastructure provisioning on push to main
   - Manual workflow dispatch for controlled deployments
   - Infrastructure destruction capabilities
...
- GitHub repository secrets configured (see GitHub Actions Setup)
...
1. **Push to main branch**: Automatically triggers infrastructure provisioning and application deployment
2. **Manual deployment**: Use "Actions" tab → "Infrastructure Pipeline" → "Run workflow"
3. **Manual destruction**: Use "Actions" tab → "Infrastructure Pipeline" → "Run workflow" (select destroy option)
...
1. **HCLOUD_TOKEN**: Your Hetzner Cloud API token
2. **HETZNER_RUNNER_KEY**: SSH private key for accessing the runner server
...
- **Secrets**: HCLOUD_TOKEN, HETZNER_RUNNER_KEY
...
1. **Automatic**: Push to `main` branch triggers infrastructure provisioning
2. **Manual**: Use "Actions" tab → "Infrastructure Pipeline" → "Run workflow"
3. **Destroy**: Manual dispatch with destroy option
...
4. Select "Destroy Infrastructure" option
